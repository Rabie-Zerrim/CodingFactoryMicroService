...
export class EntrepriseService {
  ...
  assignEntrepriseToUser(idEntreprise: number, idUser: number): Observable<any> {
    const url = `${this.baseUrl}/assignEntreprise/${idEntreprise}/toUser/${idUser}`;
    return this.http.post(url, null).pipe(
      tap(() => console.log(`Assigned Entreprise ${idEntreprise} to User ${idUser}`)), // Log the success
      catchError(this.handleError)
    );
  }
